...
final class RouterPerformanceTests: XCTestCase {
    ...
    func testCaseSensitivePerformanceExtendable() throws {
        let router = ExtendableRouter<String>()
        for letter in ["a", "b", "c", "d", "e" , "f", "g"] {
            router.register(letter, at: [
                ConstantComponent(value: letter),
                ParameterComponent(parameterName: "\(letter)_id")
            ])
        }
        
        measure {
            for _ in 0..<1_000_000 {
                var context = RoutingContext(["a", "42"])
                _ = router.route(context: &context)
            }
        }
    }
    ...
    struct CustomRoutingComponentTwo: RoutableComponent {
        func check(_ context: RoutingContext) -> Bool {
            return context.get(CustomRoutingKey.self) ?? false
        }
        
        let identifier: String = "custom component two"
    }
    
    func testMinimalRouterCaseInsensitivePerformanceExtendable() throws {
        ...
        for letter in ["a"] {
            ...
            router.register("test", at: [CustomRoutingComponent()])
            router.register("test", at: [CustomRoutingComponentTwo()])
        }
        
        measure {
            var context = RoutingContext(["a"])
            for _ in 0..<1_000_000 {
                _ = router.route(context: &context)
            }
        }
